...
const isLoggedIn = middleware(async ({ next, ctx }) => {
  ...
  if (!token) throw new TRPCError({ code: 'UNAUTHORIZED', message: 'Token is required' });
  
  const key = process.env.ACCESS_TOKEN_SECRET;
  if (!key) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Server configuration error' });
