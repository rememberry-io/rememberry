...
export async function checkIfMemberIsAdmin(userId:string, peerId:string): Promise<boolean>{
    const adminStatus = await database
        .select({isAdmin: schema.Users_Peers.is_peer_admin})
        .from(schema.Users_Peers)
        .where(and(eq(schema.Users_Peers.user_id, userId), eq(schema.Users_Peers.peer_id, peerId)))
...
export async function kickMember(kickedUserId:string, peerId:string){
    const res = database
        ...
        .where(and(eq(schema.Users_Peers.user_id, kickedUserId), eq(schema.Users_Peers.peer_id, peerId)))
        .returning()
    return res
}