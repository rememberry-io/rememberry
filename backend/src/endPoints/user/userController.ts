...
export async function controlUserUpdateById(newPassword:string, userEmail:string, username:string, userId:string) {
  const updateCredentials = await userModel.fetchUpdateCredentials(userEmail, username, userId);
  const [errorCheck, unUsed] = checkUpdateCredentials(updateCredentials, userEmail, username);
  if(errorCheck){
    return [errorCheck, null]
  }
  const salt = await bcrypt.genSalt(10);
  const hashedPwd = await bcrypt.hash(newPassword, salt);
  const [error, res] = await userModel.updateUserById(username, userEmail, userId, hashedPwd);
  if(error){
    return [error, null]
  }
  return [null, res];
