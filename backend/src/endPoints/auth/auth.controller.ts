...
class LuciaAuthentication implements AuthenticationController {
  ...
  async register(registerInput: RegisterInput): Promise<TRPCStatus<AuthOutput>> {
    ...
    try {
      ...
      console.log(e)
      return [new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "TBD" }), null] as const
    }
  }
  async login(input: LoginInput): Promise<TRPCStatus<AuthOutput>> {
    ...
    try {
      ...
      if (
        ...
        return [new TRPCError({ code: "BAD_REQUEST", message: "Incorrect email or password" }), null] as const
      }
      return [new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "TBD" }), null] as const
    }
  }
  async logout(input: LogoutInput): Promise<TRPCStatus<AuthOutput>> {
    const authRequest = this.auth.handleRequest(input.opts.ctx.req, input.opts.ctx.res)
