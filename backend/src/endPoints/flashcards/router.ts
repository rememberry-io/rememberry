...
export const flashcardRouter = router({
    ...
    getLearnableCardsFromStack: publicProcedure.input(z.string()).query(async(opts) => {
        const res = await flashcardController.controlGetLearnableFlashcardsFromStack(opts.input, opts.ctx.db)
        return res 
    }),

    getAllCardsFromParentAndChilds: publicProcedure.input(z.string()).query(async(opts) => {
        const res = await flashcardController.controlgetAllFlashcardsFromStackAndChildStacks(opts.input, opts.ctx.db)
        return res
    }),

    getLearnableCardsFromStackAndChilds: publicProcedure.input(z.string()).query(async(opts) => {
        const res = await flashcardController.getLearnableFlashcardsFromStackAndChilds(opts.input, opts.ctx.db)
        return res 
    })
})
