...
export async function controlCreateFlashcard(flashcard: types.Flashcards) {
  if(flashcard.frontside_media_link || flashcard.backside_media_link){
    const [errorCheck, newFlashcard] = await flashcardModels.createBasicFlashcard(flashcard)
    const [secondErrorCheck, media] = await flashcardModels.createMedia(flashcard)
    ...
    const [errorCheck, res] = await flashcardModels.createBasicFlashcard(flashcard)
    if(errorCheck){
      return [errorCheck, null]
    }
    return [null, res] 
...
export async function controlGetAllFlashcardsFromStack(stackId: string) {
  const [errorCheck, res] = await flashcardModels.getAllFlashcardsFromStack(stackId);
  if(errorCheck){
    return [errorCheck, null]
  }
  return [null, res];
...
export async function controlGetLearnableFlashcardsFromStack(stackId: string) {
  const [errorCheck, res] = await flashcardModels.getLearnableFlashcardsFromStack(stackId);
  if(errorCheck){
    return [errorCheck, null]
  }
  return [null, res];
...
export async function getLearnableFlashcardsFromStackAndChilds(
  ...
  const [errorCheck, res] = await flashcardModels.getLearnableFlashcardsFromStackAndChilds(stackId);
  if(errorCheck){
    return [errorCheck, null]
  }
  return [null, res];
