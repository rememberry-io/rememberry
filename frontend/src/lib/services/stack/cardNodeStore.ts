...
import { Edge, EdgeChange, Node, NodeChange, OnEdgesChange, OnNodesChange, applyEdgeChanges, applyNodeChanges } from "reactflow"
  ...
  actions: {
    ...
    addChildNode: (parentNode: CardNode, flashcard: CardNode) => {
      ...
      if (parentNode.type === "stack") {
        set({ nodes: [...get().nodes, newFlashcard], edges: [...get().edges, newEdge] })
      }
      const updatedNodes = allNodes.map(mappedNode =>
        mappedNode.id === parentNode.id ? { ...mappedNode, type: "stack" } : mappedNode
      );
